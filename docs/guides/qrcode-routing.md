...
<body>
  ...
  <script>
    ...
    if (embed) {
      // render the QR Code with custom routing
      embed.startQRCode({
        ...
        // this can occur if for some reason the QR code could not be rendered
        console.error(err);
      });
    }
  </script>
...
You can combine URL routing with other QR code styling options:
...
Custom QR code routing is useful for:
- Maintaining consistent branding by directing users to your own domain
- Integrating with existing e-commerce platforms
- Tracking user engagement through your own analytics
- Providing a custom mobile experience with additional features
- A/B testing different mobile experiences
