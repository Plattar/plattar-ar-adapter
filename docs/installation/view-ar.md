...
Plattar supports swapping between the 3D viewer and the QRCode, but due to the load times it can be easier to maintain two embeds, one for the Viewer and one for the QRCode. The [next doc](installation/maintain-viewer-and-qr.md) will explain how to do that, but for now we will use the legacy functions to do the switch.
...
As AR only works on supported devices, here is a video of the user experience opening the webpage on a desktop computer which displays the QR code to open the AR experience on the mobile device.

![AR Demo](../images/ARDemo.gif)
...
[Go to next step](installation/maintain-viewer-and-qr.md)
