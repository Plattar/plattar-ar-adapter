...
### Steps to Get Variation ID

1. Navigate to the editor of a scene you'd like to get the `variation-id` from

2. Select an object in the scene you'd like to change to a variation

   ![Navigating to Variation Editor](../images/NavigatingVariantEdit.jpg)

3. In the variation Editor, select the variation and copy the `Variation ID`. Use this ID to change between variations either by updating the attribute or using the JavaScript function.

   ![Getting the Variation ID](../images/VariantID.jpg)

## Changing Variation Using Attribute

There are two ways to change a variation in a scene: either by changing the [attribute](installation/api-reference.md) of the embed or using a JavaScript [function](installation/api-reference.md).

In the first example, we'll use attributes to change between `Variation ID`s.

### Step 1: Setup Basic HTML

First, just like in the basic example we'll get the embed tag through the CMS and install the plattar plugin through a script tag. (If you need a refresher, you can go back to the [basic example](installation/loading-scene.md))
...
Just like in the changing scene [example](installation/changing-scene.md) we'll use the `value` in our buttons to hold our `variation-id` for changing between variations in shells, lenses and microphones:
...
<iframe height="600" style="width: 100%;" scrolling="no" title="Changing Variation Using Attribute" src="https://codepen.io/plattar/embed/mybzxVK?default-tab=js%2Cresult&editable=true" frameborder="no" loading="lazy" allowtransparency="true" allowfullscreen="true">
...
Unlike the previous example which uses `setAttribute(attribute, value)`, we'll instead use a [function](installation/api-reference.md) from the plugin, `viewer.messenger.selectVariationID(variationID:string|Array<string>)`:
...
<iframe height="600" style="width: 100%;" scrolling="no" title="Changing variation Using Function" src="https://codepen.io/plattar/embed/RNbeQdx?default-tab=js%2Cresult&editable=true" frameborder="no" loading="lazy" allowtransparency="true" allowfullscreen="true">
...
[Go to next step](installation/selecting-variation-sku.md)
